$(function(){
    ...
    function labnolIframe() {
        ...
        var embed = "https://www.youtube.com/embed/ID?autoplay=1&controls=1&loop=1&modestbranding=1&rel=0&showinfo=0;";
        embed = embed.replace("ID", this.dataset.id);
        if(this.dataset.query) {
            embed=embed+"&"+this.dataset.query;
        }
...
$('.RYPP-playlist').hide();
    ...
    $("#loadMore").on('click', function (e) {
        e.preventDefault();
        if($(this).text()===$(this).attr('data-loadmore')){
            $(this).text($(this).attr('data-showless'));
        } else {
             $(this).text($(this).attr('data-loadmore'));

        }
              $('.RYPP-playlist').toggle();
        $('html,body').animate({
            scrollTop: $(this).offset().top
        }, 1500);
    });

var width = $('.widthcontrol').find('iframe').width();
   var height = height = 16/9 * width;
   $(window).resize(

       function(){
 if($(window).width()>639){
     $('.RYPP-playlist').attr('style','min-height:'+$('.RYPP-video').find('iframe').height()+'px') ;
...
function onYouTubeIframeAPIReady() {
  ...
  $('[data-rypp]').each(function(idx, el) {
    $(el)[0].rypp_data_obj.onYTIframeAPIReadyCallback();
      if($('.RYPP-video').find('iframe').length>0){
           if($(window).width()>639){
    $('.RYPP-playlist').attr('style','min-height:'+$('.RYPP-video').find('iframe').height()+'px') ;
                   $('.RYPP-playlist').attr('style','max-height:'+$('.RYPP-video').find('iframe').height()+'px') ;
           }
      }
  });
}

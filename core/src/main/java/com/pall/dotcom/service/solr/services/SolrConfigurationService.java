...
import com.pall.dotcom.service.solr.exception.AEMSolrSearchException;
...
import static com.pall.dotcom.service.solr.services.SolrConfigurationServiceAdminConstants.*;

@Component(label = "AEM Solr Search - Solr Configuration Service", description = "A service for configuring Solr", immediate = true, metatype = true)
...
/**
 * SolrConfigurationService provides a services for setting and getting Solr
...
public class SolrConfigurationService {
    ...
    public static final String[] DEFAULT_ALLOWED_REQUEST_HANDLERS = new String[] { "/select", "/pall-search" };
    ...
    @Property(name = PROXY_ENABLED, value = "true", label = "Enable Proxy", description = "Enable Proxy. Must be either 'true' or 'false'")
    private Boolean proxyEnabled = false;

    @Property(name = PROXY_URL, value = "http://localhost:4502/apps/solr/proxy", label = "Proxy URL", description = "Absolute proxy URL")
    private String proxyUrl;

    @Property(name = ALLOWED_REQUEST_HANDLERS, value = { "/select",
            "/pall-search" }, label = "Allowed request handlers", description = "Whitelist of allowed request handlers")
    private String[] allowedRequestHandlers;

    @Property(name = SOLR_MODE, label = "Solr Mode", value = "Standalone", options = {
            @PropertyOption(name = "Standalone", value = "Standalone"),
            @PropertyOption(name = "SolrCloud", value = "SolrCloud") })
    private String solrMode;

    @Property(name = SOLR_ZKHOST, value = {
            "localhost:9983" }, label = "SOLR ZooKeeper Hosts", description = "A comma delimited list of ZooKeeper hosts "
                    + "using the same format expected by CloudSolrClient")
    private String solrZKHost;

    @Property(name = SOLR_MASTER, value = {
            "http://localhost:8888/solr" }, label = "Master Server", description = "The master Solr server is formatted as follows: <scheme>://<host>:<port>/<solr context>"
                    + " (i.e., http://solr1.example.com:8080/solr).")
    private String solrMaster;

    @Property(name = SOLR_SLAVES, value = {
            "" }, label = "An optional set of zero or more Solr slave servers.", description = "This property will be considered for Solr mode = 'Standalone' only. "
    ...
    @Property(name = ALLOWED_SOLR_MASTER_QUERIES, boolValue = false, label = "Allowed Solr Master Queries", description = "This property will be considered for Solr mode = 'Standalone'. "
    ...
    private Boolean solrAllowMasterQueriesEnabled = false;
     ...
     *
     * @return a list of available cores on success, and an empty list
     *         otherwise.
     */
    ...
    /**
     * Returns all Solr fields that have <code>stored</code> set to
     * <code>true</code>.
     *
     * @param solrCore
     *            Solr core name
    ...
    public List<String> getStoredFields(String solrCore) {
        ...
        try {
            SchemaResponse.FieldsResponse fieldsResponse = fieldsSchemaRequest.process(solrClient, solrCore);
    ...
    /**
     * Returns all Solr fields that have <code>indexed</code> set to
     * <code>true</code>
     *
     * @param solrCore
     *            Solr core name
    ...
    /**
     * Returns all Solr fields that have <code>indexed</code> set to
     * <code>true</code>. This implementation uses the
     * <code>LukeRequestHandler</code>, as such this method will return dynamic
     * fields as well.
     *
     * @param solrCore
     *            Solr core name
    ...
    private synchronized void resetService(final Map<String, Object> config) {
        ...
        proxyUrl = config.containsKey(PROXY_URL) ? (String) config.get(PROXY_URL) : DEFAULT_PROXY_URL;
        proxyEnabled = config.containsKey(PROXY_ENABLED) ? Boolean.parseBoolean((String) config.get(PROXY_ENABLED))
                : true;
        allowedRequestHandlers = config.containsKey(ALLOWED_REQUEST_HANDLERS)
                ? (String[]) config.get(ALLOWED_REQUEST_HANDLERS) : DEFAULT_ALLOWED_REQUEST_HANDLERS;
        solrMode = config.containsKey(SOLR_MODE) ? (String) config.get(SOLR_MODE) : DEFAULT_SOLR_MODE;
        solrMaster = config.containsKey(SOLR_MASTER) ? (String) config.get(SOLR_MASTER) : DEFAULT_SOLR_MASTER;
        solrEndPoint = solrMaster;

        if (SOLR_MODE_SOLRCLOUD.equals(solrMode)) {
            solrZKHost = config.containsKey(SOLR_ZKHOST) ? (String) config.get(SOLR_ZKHOST) : DEFAULT_SOLR_ZKHOST;
        }
    ...
    private void configureMasterSlaveInt(Map<String, Object> config) {
        ...
        solrSlaves = config.containsKey(SOLR_SLAVES) ? (String) config.get(SOLR_SLAVES) : null;

        solrAllowMasterQueriesEnabled = config.containsKey(ALLOWED_SOLR_MASTER_QUERIES)
                ? (Boolean) config.get(ALLOWED_SOLR_MASTER_QUERIES) : DEFAULT_ALLOW_MASTER_QUERIES;
    ...
    private SolrClient getSolrClient(String solrOperation) {
        ...
        if (null != existingSolrServer) {
            LOG.info("Returning existing instance of Solr Server {} for operation '{}'", existingSolrServer,
                    solrOperation);
